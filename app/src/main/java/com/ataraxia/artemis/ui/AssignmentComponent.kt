...
fun AssignmentContent(
    ...
    val indexOfCurrentQuestion = assignmentViewModel.indexOfCurrentQuestion.observeAsState(0)
    val assignmentQuestions: List<Question> by questionViewModel.questionsForAssignment.observeAsState(
        questionViewModel.prepareQuestionsForAssignment()
    )

    val currentQuestion = assignmentQuestions[indexOfCurrentQuestion.value]

    Log.v("Current assignment questions", assignmentQuestions.forEach(::println).toString())
    ...
    Column(
        ...
        Row(
            ...
            for (skipIndex in 0..110 step 10) {
                TextButton(
                    ...
                    Text(
                        style = MaterialTheme.typography.caption,
                        text = if (skipIndex != 0) skipIndex.toString() else "1",
                        color = Color.Black
                    )
                }
            }
            ...
            Row {
                Row(
                    ...
                    IconButton(
                        ...
                        onClick = {
                            assignmentViewModel.onChangeIndexOfCurrentQuestion(0)
                        }) {
                        Icon(
                    ...
                    IconButton(
                        enabled = isButtonEnabled,
                        onClick = {
                            if (indexOfCurrentQuestion.value != 0) {
                                assignmentViewModel.onChangeIndexOfCurrentQuestion(
                                    indexOfCurrentQuestion.value - 1
                                )
                            }
                        }
                    ) {
                ...
                Row(
                    ...
                    IconButton(
                        enabled = isButtonEnabled,
                        onClick = {
                            if (indexOfCurrentQuestion.value != assignmentQuestions.size - 1) {
                                assignmentViewModel.onChangeIndexOfCurrentQuestion(
                                    indexOfCurrentQuestion.value + 1
                                )
                            }
                    ...
                    IconButton(
                        enabled = isButtonEnabled,
                        onClick = {
                            assignmentViewModel.onChangeIndexOfCurrentQuestion(assignmentQuestions.size - 1)
                        }) {
                        Icon(
        ...
        Column {
            Text(
                text = "${indexOfCurrentQuestion.value + 1}/${assignmentQuestions.size}",
